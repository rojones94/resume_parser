...
class resumeparse(object):
    ...
    def _extract_lines(raw_text):
        full_string = raw_text
        full_string = full_string.replace("\r", "\n")
        full_string = full_string.replace("\t", " ")
        full_string = re.sub(r'(?:\n\s*)+', '\n', full_string)

        # Split text blob into individual lines
        resume_lines = full_string.splitlines()
    ...
    def convert_docx_to_txt(docx_file, docx_parser):
        ...
        try:
            if docx_parser == "tika":
                ...
                logging.error(
                    'Choose docx_parser from tika or docx2txt :: ' + str(e)+' is not supported')
                return [], " "
        ...
        try:
            ...
            resume_lines = resumeparse._extract_lines(text)
    ...
    def convert_pdf_to_txt(pdf_file):
        ...
        try:
            ...
            return [], " "
        try:
            resume_lines = resumeparse._extract_lines(raw_text)
    ...
    def convert_rtf_to_txt(rtf_file):
        """
        A utility function to convert a Rich Text Format to raw text.

        :param input_pdf_path: Path to the .rtf file which should be converted
        :return: The text contents of the rtf
        :rtype: str
        """

        try:
            raw_text = parser.from_file(rtf_file, service='text')['content']
        except RuntimeError as e:
            logging.error('Error in tika installation:: ' + str(e))
            logging.error('--------------------------')
            logging.error('Install java for better result ')
    ...
    def read_file(file, docx_parser="tika"):
        ...
        if file.endswith('docx') or file.endswith('doc'):
            if file.endswith('doc') and docx_parser == "docx2txt":
                docx_parser = "tika"
                logging.error(
                    "doc format not supported by the docx2txt changing back to tika")
            resume_lines, raw_text = resumeparse.convert_docx_to_txt(
                file, docx_parser)
        elif file.endswith('pdf'):
            resume_lines, raw_text = resumeparse.convert_pdf_to_txt(file)
        elif file.endswith('rtf'):
            resume_lines, raw_text = resumeparse.convert_rtf_to_txt(file)
